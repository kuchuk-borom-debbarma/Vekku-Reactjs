...
const ContentView: React.FC<ContentViewProps> = ({ content, trigger }) => {
  ...
  const handleKeywordClick = async (keyword: string) => {
    try {
      // Create tag + Link immediately
      const res = await api.post("/tag", { name: keyword, semantic: keyword });
      const tag = res.data;
      await api.post(`/content/${content.id}/tags`, { tagIds: [tag.id] });
      await fetchTagsAndSuggestions();
    } catch (err) {
  ...
  // ... (RemoveTag, HandleToggleTag, HandleSaveManagedTags remain the same) ...
  // Update selected IDs when active tags change
  useEffect(() => {
     setSelectedTagIds(activeTags.map(t => t.id));
  }, [activeTags]);
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                {selectedSuggestionIds.length > 0 && (
                  <button onClick={handleAddSelectedSuggestions} disabled={isAddingSuggestions} className="text-xs flex items-center gap-1 bg-indigo-600 text-white hover:bg-indigo-700 font-medium px-3 py-1.5 rounded-full shadow-sm transition-all disabled:opacity-50">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {displayedKeywordSuggestions.map((kw) => (
                        <button
                          key={kw.name}
                          onClick={() => handleKeywordClick(kw.name)}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium border shadow-sm transition-all bg-white text-purple-700 border-purple-200 hover:shadow-md hover:border-purple-300 hover:bg-purple-50"
                        >
                          <Plus size={12} />
                          {kw.name}
                        </button>
